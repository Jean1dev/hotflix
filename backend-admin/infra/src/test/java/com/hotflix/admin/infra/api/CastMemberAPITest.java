...
public class CastMemberAPITest {
    ...
    public void givenValidParams_whenCallListCastMembers_shouldReturnIt() throws Exception {
        ...
        response.andExpect(status().isOk())
                ...
                .andExpect(jsonPath("$.itens[0].id", equalTo(aMember.getId().getValue())))
                .andExpect(jsonPath("$.itens[0].name", equalTo(aMember.getName())))
                .andExpect(jsonPath("$.itens[0].type", equalTo(aMember.getType().name())))
                .andExpect(jsonPath("$.itens[0].created_at", equalTo(aMember.getCreatedAt().toString())));
