...
import com.hotflix.admin.application.genre.create.CreateGenreOutput;
...
import com.hotflix.admin.application.genre.update.UpdateGenreOutput;
...
public class GenreAPITest {
    ...
    public void givenAnInvalidName_whenCallsCreateGenre_shouldReturnNotification() throws Exception {
        ...
        when(createGenreUseCase.execute(any()))
                .thenThrow(new NotificationException("Error", NotificationHandler.create(new DomainError(expectedErrorMessage))));
    ...
    public void givenValidParams_whenCallsListGenres_shouldReturnGenres() throws Exception {
        ...
        response.andExpect(status().isOk())
                ...
                .andExpect(jsonPath("$.itens[0].id", equalTo(aGenre.getId().getValue())))
                .andExpect(jsonPath("$.itens[0].name", equalTo(aGenre.getName())))
                .andExpect(jsonPath("$.itens[0].is_active", equalTo(aGenre.isActive())))
                .andExpect(jsonPath("$.itens[0].created_at", equalTo(aGenre.getCreatedAt().toString())))
                .andExpect(jsonPath("$.itens[0].deleted_at", equalTo(aGenre.getDeletedAt().toString())));
