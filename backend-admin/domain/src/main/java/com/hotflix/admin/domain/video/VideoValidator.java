...
public class VideoValidator extends Validator {
    ...
    private void checkTitleConstraints() {
        ...
        if (length > TITLE_MAX_LENGTH) {
            this.validationHandler().append(new DomainError("'title' must be between 1 and 255 characters"));
        }
    }
    ...
    private void checkDescriptionConstraints() {
        ...
        if (length > DESCRIPTION_MAX_LENGTH) {
            this.validationHandler().append(new DomainError("'description' must be between 1 and 4000 characters"));
        }
    }

    private void checkLaunchedAtConstraints() {
        if (this.video.getLaunchedAt() == null) {
            this.validationHandler().append(new DomainError("'launchedAt' should not be null"));
        }
    }

    private void checkRatingConstraints() {
        if (this.video.getRating() == null) {
            this.validationHandler().append(new DomainError("'rating' should not be null"));
        }
    }
}
